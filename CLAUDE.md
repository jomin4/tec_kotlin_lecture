...
2. **시각 자료** — 핵심을 도표/그림 1장으로. 텍스트로 다 쓰지 말고 그림으로 요약.
   - **원칙(2026-07-13 사용자 요청): 설명을 박스에 다시 적지 않는다.** 이론 중 "시각화할 가치가 있는 핵심 메커니즘 1개"를 내가 판단해 고르고,
     ...
     **재사용 템플릿 킷 필수 사용**: Spring 도표는 `docs/spring-analysis/viz-template.html`의 `<style>`(토큰·테마·컴포넌트 CSS)을 복사해, 헤더/코드앵커/`.tl`타임라인/`.seq`시퀀스/`.proxy`래핑/`.graph`그래프/`.map`대응표 컴포넌트를 **조립**해 만든다. 즉흥 제작(텍스트 박스 회귀) 금지. 킷 갱신 시 이 파일을 확장.
