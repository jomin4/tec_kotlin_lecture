...
2. **시각 자료** — 핵심을 도표/그림 1장으로. 텍스트로 다 쓰지 말고 그림으로 요약.
   - **원칙(2026-07-13 사용자 요청): 설명을 박스에 다시 적지 않는다.** 이론 중 "시각화할 가치가 있는 핵심 메커니즘 1개"를 내가 판단해 고르고,
     ...
     **⚠️ (Spring 분석 트랙, 2026-07-13 사용자 요청) "텍스트 박스 나열" 금지 → 다이어그램 문법 사용.** 유형별로: 생명주기=시간축 타임라인, 호출흐름(AOP·요청)=시퀀스(lifeline+화살표), 프록시=래핑도, DI=의존성 그래프. 플레이북·조사출처: `docs/spring-analysis/viz-playbook.md`.
     **재사용 템플릿 킷 필수 사용 = 김영한 스프링 입문 슬라이드 스타일** (2026-07-13 사용자 제공 자료 기반): Spring 도표는 `docs/spring-analysis/viz-template.html`의 `<style>`을 복사해 컴포넌트를 **조립**한다. 노드=`.jh-bean`(초록/빈)·`.jh-proxy`(주황)·`.jh-infra`(검정/Aspect)·`.jh-actor`(흰/외부), 틀=`.jh-container`(굵은실선)·`.jh-group`(점선), 연결=`.jh-arrow(.dash)`·`.jh-adown`·`.jh-step`(순번)·`.jh-x`(빨강✕), 레이아웃=`.jh-row`(좌→우 계층). 즉흥 제작(텍스트 박스 회귀) 금지.
